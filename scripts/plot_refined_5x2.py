...
def plot_final_grid_tight():
    ...
    for row, panel in enumerate(cell_data):
        ...
        if len(rec) > 0:
            ...
            ax.set_ylabel(r"$V_m$ (mV)", fontsize=LABEL_FONT_SIZE)
            ax.yaxis.set_label_coords(-0.08, y_to_axes_frac(ax, -10.0))
            if row == 0:
                ax.text(
                    0.74,
                    0.08,
    ...
    plt.tight_layout()
    out_path = os.path.join("paper", "figures", "supp_figure4_pre_i_recruitment.pdf")
