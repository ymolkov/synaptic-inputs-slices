...
def figure_4_combined_summary():
    ...
    positions = np.array([0.75, 1.25, 2.75, 3.25])
    phase_ticks = [1.0, 3.0]
    ...
    for i, group in enumerate(groups):
        ...
        for j in range(4):
            ...
            clean = values[mask]
            box_data.append(clean if len(clean) else values)
        ...
        bp = ax.boxplot(
            ...
            medianprops={"color": ERROR_COLOR, "linewidth": 1.4},
            whiskerprops={"color": ERROR_COLOR, "linewidth": 1.1},
            capprops={"color": ERROR_COLOR, "linewidth": 1.1},
        )
        for patch, color in zip(bp["boxes"], colors):
            patch.set_facecolor(color)
            patch.set_alpha(0.35)
            patch.set_edgecolor(color)
        for x, values, color in zip(positions, box_data, colors):
        ...
        if i == 0:
            legend_handles = [
                plt.Line2D([0], [0], marker="o", color="none", markerfacecolor=EXC_COLOR,
                           markeredgecolor="white", markersize=6, label="Excitation"),
                plt.Line2D([0], [0], marker="o", color="none", markerfacecolor=INH_COLOR,
                           markeredgecolor="white", markersize=6, label="Inhibition"),
            ]
...
def figure_2_four_populations():
    ...
    for i, (name, basename, start, end) in enumerate(episodes):
        ...
        ax.plot(t_aligned, ref_mapped, color=REF_COLOR, lw=2, alpha=0.8, label=r'$\int$ XII (a.u.)' if i==0 else "")
        ax.plot(t_aligned, v_norm, color=TRACE_COLOR, lw=1, alpha=0.9, label=r'$V_m$ (mV)' if i==0 else "")
    ...
    plt.tight_layout()
    plt.subplots_adjust(hspace=0.1, left=0.08) # compress space between
    fig.text(0.02, 0.5, r'$V_m$ (mV)', rotation='vertical',
             va='center', ha='center', fontsize=AXIS_LABEL_SIZE)
