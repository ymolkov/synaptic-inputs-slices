...
*   `make all`: Regenerates the population summaries and the dashboard.
...
*   `make clean`: Removes binaries and temporary files for a fresh start.
...
*   **This repository**: applies the same method to VgluT2/VGAT respiratory slice recordings, with reproducible scripts, generated summaries, and a browser for every per-recording analysis artifact.
...
*   Population summaries report `$G_{exc}/g_{leak}$` and `$G_{inh}/g_{leak}$` after IQR-based outlier filtering.
*   The dashboard keeps per-recording outputs visible alongside accepted/rejected status, cycle counts, inferred parameters, and analysis snapshots.
*   Site-level circuit summaries use the same CSV-derived population means to assign connection weights.
